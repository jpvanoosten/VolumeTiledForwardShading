...
LightingResult DoLighting( uint clusterIndex1D, Material material, float4 eyePos, float4 P, float4 N )
{
    // Counters
    uint i;
    ...
    startOffset = PointLightGrid_Cluster[clusterIndex1D].x;
    lightCount = PointLightGrid_Cluster[clusterIndex1D].y;
    
    // Iterate point lights.
    for ( i = 0; i < lightCount; ++i )
    {
        lightIndex = PointLightIndexList_Cluster[startOffset+i];
    ...
    // Get the start position and offset of the light in the light index list.
    startOffset = SpotLightGrid_Cluster[clusterIndex1D].x;
    lightCount = SpotLightGrid_Cluster[clusterIndex1D].y;

    // Iterate spot lights.
    for ( i = 0; i < lightCount; ++i )
    {
        lightIndex = SpotLightIndexList_Cluster[startOffset+i];
...
float4 main( VertexShaderOutput IN ) : SV_Target
{
    ...
    // Get the index of the current pixel in the cluster grid.
    ...
    LightingResult lit = DoLighting( clusterIndex1D, material, eyePos, IN.PositionVS, N );
