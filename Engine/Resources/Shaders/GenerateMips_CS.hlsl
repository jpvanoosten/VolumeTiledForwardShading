...
#if !defined(TEXTURE_DIMENSION)
#define TEXTURE_DIMENSION WIDTH_HEIGHT_EVEN // Assume textures with both width and height are even.
#endif
...
void main( ComputeShaderInput IN )
{
    ...
    float4 Src1 = 0.5 * ( SrcMip.SampleLevel( LinearClampSampler, UV1, SrcMipLevel ) +
                          SrcMip.SampleLevel( LinearClampSampler, UV1 + Off, SrcMipLevel ) );
#elif TEXTURE_DIMENSION == WIDTH_HEIGHT_ODD
    // > 2:1 in in both dimensions
    // Use 4 bilinear samples to guarantee we don't undersample when downsizing by more than 2x
    // in both directions.
    float2 UV1 = TexelSize * ( IN.DispatchThreadID.xy + float2( 0.25, 0.25 ) );
    float2 O = TexelSize * 0.5;
    float4 Src1 = SrcMip.SampleLevel( LinearClampSampler, UV1, SrcMipLevel );
